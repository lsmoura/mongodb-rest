                ...
                db.collection(req.params.collection, function(err, collection) {
                    ...
                    collection.insertMany(
                        ...
                        function(err, docs) {
                            ...
                            var _id = docs.insertedIds[0].toHexString();
                            res.header('Location', '/' + req.params.db + '/' + req.params.collection + '/' + _id);
                            res.status(201).json({
                                'ok': 1,
                                '_id': _id
                            });
    ...
    app.put(urlPrefix + '/:db/:collection/:id', function(req, res) {
        ...
        connection.connect(dbConnectionStr + '/' + req.params.db, function (err, db) {
            ...
            db.collection(req.params.collection, function(err, collection) {
                ...
                var options = {
                    upsert: !!req.params.upsert
                };
                collection.update(spec, req.body, options, function(err, docs) {
                    res.json({ ok: 1 });
                });
            });
        });
