            ...
            tokensDb.collection(tokensCollectionName, function (err, tokensCollection) {
                ...
                tokensCollection.findOne({ token: token }, function (err, token) {
                    ...
                    if( expirationDate <= new Date() ) {
                        tokensCollection.remove({ _id: token._id },function (err, numberOfRemovedDocs) {
                          logger.warn('Unauthorized access - token ' + token + ' expired, removing token');
                            ...
                            tokensCollection.findOne({ userId: user._id }, function (err, token) {
                                ...
                                if (token) {
                                    ...
                                    if (expirationDate > new Date() ) {
                                        ...
                                        tokensCollection.remove({ _id: token._id },function (){
                                          logger.verbose('Token removed ' + token.token + ' for user ' + user._id);
                                        });
                                    }
                                }
